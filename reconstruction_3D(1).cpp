...
Matrix<double, Dynamic, 2> matchFeatures( Matrix<double, Eigen::Dynamic, 2> featurepoints_lastimage,  Matrix<double, Eigen::Dynamic, 2>featurepoints_currentimage )
{
  ...
  Matrix<int, featsize_last_row, 2> matchedpair; // 第二个参数填为5,作为测试
  ...
  for(int i = 0; i < featsize_last_row; i=i+1) //check matchedpair one by one
  {
    ...
    Matrix<int, featsize_last_row, 2> matchedpair;
    matchedpair(i,0)= i;
    ...
    //上面，我默认为你是创建一个新的矩阵
    Eigen::Matrix<int, Eigen::Dynamic, 2> featurepoints_lastimage;

    Eigen::Matrix<int, Eigen::Dynamic> residuals; //find color
